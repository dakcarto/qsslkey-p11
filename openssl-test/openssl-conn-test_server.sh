...
if [ "${CONDA_PREFIX}" != "/opt/mc3/envs/qgis310-deps-openssl" ]; then
  conda activate qgis310-deps-openssl
fi
...
openssl s_server \
  -accept openssl.planet.local:4443 \
  -4 \
  -Verify 5 \
  -verify_return_error \
  -cert ${TEST_CERTS}/server-cert.pem \
  -key ${TEST_CERTS}/server-key.pem \
  -CAfile ${TEST_CERTS}/server-ca.pem \
  -build_chain \
